...
for i = 1:sample_size
    ...
    exp_date = [];
    for j = 1:2:5*2 % collecting for all plane, one sample
        ...
        mean_aplane_terk = mean(terk_without_nan(:,3),'all'); % used to calculate ratio
...
%         xyz_locs_perk = [xyz_locs_perk;perk_without_nan(:,5),perk_without_nan(:,6),ones(size(perk_without_nan,1),1)*k]; % xyz locations
    ...
    if org_group(end-1) == 'DMSO'
        ...
        % by samples, plane recorded in third column
...
% _samples_planes, by sample and by plane
% _planes, by plane, all samples 

% DMSO_all_pt = []; TAK_all_pt = []; JNJ_all_pt = []; MK_all_pt = []; MKTAK_all_pt = []; MKJNJ_all_pt = [];
% group = {DMSO_samples_planes, JNJ_samples_planes, TAK_samples_planes, MK_samples_planes, MKJNJ_samples_planes, MKTAK_samples_planes};
% for j = 1:size(group,2)
%     m = 1;
%     a_group_all_pt = [];
%     for k = 1:size(group{j},1)
%         for l = 1:5
%             a_sample_a_plane = mean(group{j}{k,l},'all');
%             a_group_all_pt = [a_group_all_pt,a_sample_a_plane];
%             m = m + 1;
%         end
%     end
%     groups_all_pt{j} = a_group_all_pt;
% end

% JNJ n TAK
DMSO_all_pt = [DMSO_planes{1};DMSO_planes{2};DMSO_planes{3};DMSO_planes{4};DMSO_planes{5}];
JNJ_all_pt = [JNJ_planes{1};JNJ_planes{2};JNJ_planes{3};JNJ_planes{4};JNJ_planes{5}];
TAK_all_pt = [TAK_planes{1};TAK_planes{2};TAK_planes{3};TAK_planes{4};TAK_planes{5}];
jnjtak_group_all_pt = {DMSO_all_pt,JNJ_all_pt,TAK_all_pt};

cs_data = [DMSO_all_pt;JNJ_all_pt;TAK_all_pt];
cs_group = [ones(1,length(DMSO_all_pt))*1, ones(1,length(JNJ_all_pt))*2, ones(1,length(TAK_all_pt))*3];
[p_dmso_cs,tbl_dmso_cs,stats_dmso_cs] = anova1(cs_data',cs_group,'off');
p_multcomp_dmso_cs = multcompare(stats_dmso_cs,"CriticalValueType","bonferroni");

% figure();
% for g = 1:3
% sort_all_pt = sort(jnjtak_group_all_pt{g},'ascend');
% one_cd = cumsum(sort_all_pt);
% cumulative_frac = one_cd ./ one_cd(end);
% norm_cf = normalize(cumulative_frac,'range',[0 1]);
% plot(sort_all_pt,norm_cf); hold on
% end

% MKJNJ n MKTAK
MK_all_pt = [MK_planes{1};MK_planes{2};MK_planes{3};MK_planes{4};MK_planes{5}];
MKJNJ_all_pt = [MKJNJ_planes{1};MKJNJ_planes{2};MKJNJ_planes{3};MKJNJ_planes{4};MKJNJ_planes{5}];
MKTAK_all_pt = [MKTAK_planes{1};MKTAK_planes{2};MKTAK_planes{3};MKTAK_planes{4};MKTAK_planes{5}];
mkjnjmktak_group_all_pt = {MK_all_pt,MKJNJ_all_pt,MKTAK_all_pt};

mk_cs_data = [MK_all_pt;MKJNJ_all_pt;MKTAK_all_pt];
mk_cs_group = [ones(1,length(MK_all_pt))*1, ones(1,length(MKJNJ_all_pt))*2, ones(1,length(MKTAK_all_pt))*3];
[p_mk_cs,tbl_mk_cs,stats_mk_cs] = anova1(mk_cs_data',mk_cs_group,'off');
p_multcomp_mk_cs = multcompare(stats_mk_cs,"CriticalValueType","bonferroni");

% figure();
% for g = 1:3
% sort_all_pt = sort(mkjnjmktak_group_all_pt{g},'ascend');
% one_cd = cumsum(sort_all_pt);
% cumulative_frac = one_cd ./ one_cd(end);
% norm_cf = normalize(cumulative_frac,'range',[0 1]);
% plot(sort_all_pt,norm_cf); hold on
...
%% mean of intensity based on the x-coordinates
group_x_coor = {DMSO_xyz,JNJ_xyz,TAK_xyz,MK_xyz,MKJNJ_xyz,MKTAK_xyz};
group_int = {DMSO_samples_planes,JNJ_samples_planes,TAK_samples_planes,MK_samples_planes,MKJNJ_samples_planes,MKTAK_samples_planes};
for o = 1:size(group_x_coor,2)
    for p = 1:size(group_x_coor{o},1) % sample
        for u = 2:4
            plane_x = find(group_x_coor{o}{p,2}(:,3) == u);
            x_coor_round_half = ceil((group_x_coor{o}{p,2}(plane_x,1) * 5)) / 5;
%             x_coor_round_half = round(group_x_coor{o}{p,2}(plane_x,1));            
            unique_x_coor = unique(x_coor_round_half);
            sample_int = zeros(length(unique_x_coor));
            int_by_point = 0;
            x_length = 1:0.2:7; % change after changing x_coor_round_half!!
            for i = 1:length(x_length)
                ind_zero_one = x_coor_round_half == x_length(i);
                ind_by_point = find(ind_zero_one == 1);

                if all(ind_by_point == 0)
                    int_by_point(i) = 0;
                else
                    sam_pl_int = group_int{o}{p,u};
                    int_by_point(i) = mean(sam_pl_int(ind_by_point),'all');
                end
            end
            int_by_xcoor_byplane{u-1} = movmean(int_by_point,6);
        end
        int_by_xcoor_bysample{p} = int_by_xcoor_byplane;
    end
    allsamp_same_plane_two = [zeros(1,length(x_length))];allsamp_same_plane_three = [zeros(1,length(x_length))];allsamp_same_plane_four = [zeros(1,length(x_length))];
    for p = 1:size(group_x_coor{o},1)
        allsamp_same_plane_two = [allsamp_same_plane_two; int_by_xcoor_bysample{p}{1}]; 
        allsamp_same_plane_three = [allsamp_same_plane_three; int_by_xcoor_bysample{p}{2}]; 
        allsamp_same_plane_four = [allsamp_same_plane_four; int_by_xcoor_bysample{p}{3}];
    end
    group_int_by_xcoor{o,1} = allsamp_same_plane_two(2:end,:);
    group_int_by_xcoor{o,2} = allsamp_same_plane_three(2:end,:);
    group_int_by_xcoor{o,3} = allsamp_same_plane_four(2:end,:);
end

% group_int_by_xcoor, row = group, column = plane 2 3 4
for o = 1:size(group_int_by_xcoor,1)
    for p = 1:3 % plane
        one_line = group_int_by_xcoor{o,p};
        mean_xcoor{o,p} = mean(one_line,1); % zscore
        std_xcoor{o,p} = std(one_line,[],1);
        sem_xcoor{o,p} = std_xcoor{o,p} ./ sqrt(length(std_xcoor{o,p}));
    end
end

choose_this_mean = mean_xcoor{6,1};
choose_this_sem = sem_xcoor{6,1};
x_coor_length = x_length;
figure();
plot(x_coor_length,choose_this_mean,'LineWidth',1); hold on;
fill([x_coor_length, fliplr(x_coor_length)], [choose_this_mean + choose_this_sem, fliplr(choose_this_mean - choose_this_sem)], 'b', 'FaceAlpha', 0.2, 'EdgeColor', 'none');
